...
namespace SIGE.Core.SQLFactory
{
    ...
    {
        public static string ListaRelatoriosEconomia(RelatorioEconomiaRequest relatorio)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("SELECT forn.Id AS 'FornecedorId', relatorio.Id AS 'Id', contrato.Id AS 'ContratoId', contrato.DscGrupo AS 'DescGrupo', forn.Nome AS 'DescFornecedor', IFNULL(relatorio.Fase, 0) AS 'Fase', relatorio.Competencia AS 'Competencia', null AS 'DataEmissao'");
            ...
            builder.Append("LEFT JOIN RelatoriosEconomia relatorio on relatorio.ContratoId = contrato.Id ");
            builder.Append("ORDER BY emp.NomeFantasia");
            return builder.ToString();
        }

        public static string ValoresRelatoriosEconomia(Guid contratoId, DateTime competencia, Guid? empresaId)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("SELECT contrato.TipoEnergia as 'TipoEnergia', CONCAT(empresa.Logradouro, ', ', empresa.Bairro, ', ', empresa.Cidade, '/', empresa.Estado) AS 'DescEndereco', empresa.Cnpj AS 'NumCnpj', empresa.NomeFantasia AS 'DescEmpresa', empresa.Id AS 'EmpresaId', total.Icms, total.Proinfa, fornecedor.Id AS 'FornecedorId', contrato.DataBase AS 'DataBase', IFNULL(contrato.TakeMinimo,0) AS 'TakeMinimo', IFNULL(contrato.TakeMaximo,0) AS 'TakeMaximo', IFNULL(relatorio.Fase, 0) AS 'Fase', contrato.Id as 'ContratoId', contrato.DscGrupo AS 'DescGrupo', fornecedor.Nome AS 'DescFornecedor', relatorio.Id AS 'Id', relatorio.Competencia AS 'Competencia', contrato.Numero AS 'NumContrato', IFNULL(valorAnual.ValorUnitarioKwh, 0) AS 'ValorUnitarioKwh', IFNULL(valorMensal.EnergiaContratada,0) AS 'EnergiaContratada', IFNULL(valorMensal.HorasMes,0) AS 'HorasMes', IFNULL(total.ConsumoAtivo, 0) AS 'TotalMedido', relatorio.DataEmissao AS 'DataEmissao', relatorio.Observacao, contrato.DataVigenciaInicial, contrato.DataVigenciaFinal ");
            builder.Append("FROM Contratos contrato ");
            builder.Append("INNER JOIN ContratoEmpresas contratoEmpresa ON contratoEmpresa.ContratoId = contrato.Id ");
            ...
            builder.Append(string.Format("AND DATE_FORMAT(valorAnual.DataVigenciaInicial, '%Y-%m-01') <= '{0}' AND DATE_FORMAT(valorAnual.DataVigenciaFinal, '%Y-%m-01') >= '{0}' ", competencia.ToString("yyyy-MM-01")));
            builder.Append("INNER JOIN ValoresMensaisContrato valorMensal ON valorMensal.ValorAnualContratoId = valorAnual.Id ");
            builder.Append(string.Format("AND DATE_FORMAT(valorMensal.Competencia, '%Y-%m-01') = '{0}' ", competencia.ToString("yyyy-MM-01")));
            ...
            builder.Append(string.Format("	WHERE medicao.SubTipo LIKE 'L' AND DATE_FORMAT(medicao.Periodo, '%Y-%m-01') = '{0}' ", competencia.ToString("yyyy-MM-01")));
            if (empresaId != null)
            ...
            builder.Append(string.Format("AND DATE_FORMAT(total.MesReferencia, '%Y-%m-01') = '{0}' ", competencia.ToString("yyyy-MM-01")));
            builder.Append("LEFT JOIN RelatoriosEconomia relatorio ON relatorio.ContratoId = contrato.Id ");
