...
namespace SIGE.Core.SQLFactory {
    public static class RelatorioEconomiaFactory {
        ...
        public static string ObterParametrosRelatorioEconomia() {
            ...
            builder.AppendLine("    vm.Id AS ValorMensalId,");
            builder.AppendLine("    vm.Proinfa,");
            builder.AppendLine("    vm.Icms");
            ...
            builder.AppendLine("LEFT JOIN ValoresMensaisPontoMedicao vm");
            builder.AppendLine("    ON vm.PontoMedicaoId = @PontoMedicaoId");
            builder.AppendLine("    AND DATE_FORMAT(vm.MesReferencia, '%Y-%m-01') = DATE_FORMAT(@MesReferencia, '%Y-%m-01')");
            builder.AppendLine("    AND vm.Ativo = TRUE");
            builder.AppendLine("    AND vm.DataExclusao IS NULL");
