...
namespace SIGE.Controller.Externo {
    ...
    public class EMailController(IEmailService service) : ControllerBase {
        ...
        [ProducesResponseType(typeof(Response), 401)]
        [ProducesResponseType(typeof(Response), 500)]
        public async Task<IActionResult> SendEmailEconomia([FromBody] EmailDataDto req) =>
            Ok(await _service.SendEmailEconomia(req));
        ...
        [ProducesResponseType(typeof(Response), 401)]
        [ProducesResponseType(typeof(Response), 500)]
        public async Task<IActionResult> OpenEmailEconomia([FromRoute] Guid id) {
            _ = await _service.OpenEmail(id);
            var pixel = new byte[]
                {
                        71, 73, 70, 56, 57, 97, 1, 0,
