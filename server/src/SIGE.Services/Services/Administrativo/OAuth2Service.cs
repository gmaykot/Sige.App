...
namespace SIGE.Services.Services.Administrativo {
    ...
    public class OAuth2Service(
        ICustomLoggerService loggerService,
        ...
        public async Task<TokenDto> Introspect(Guid req) {
            var token = await _appDbContext
                .Tokens.Include(t => t.Usuario)
                .FirstOrDefaultAsync(t => t.Id.Equals(req));
        ...
        public async Task<Response> Login(LoginRequest req) {
            ...
            await _loggerService.LogAsync(
                ...
                $"Login solicitado por {req.Email}."
            );

            if (string.IsNullOrEmpty(req.Email) || string.IsNullOrEmpty(req.Password))
                return ret.SetBadRequest()
                    .AddError(ETipoErro.ATENCAO, "Os dados obrigatorios devem ser preenchidos.");

            var usuario = await _appDbContext
                .Usuarios.AsNoTracking()
                .FirstOrDefaultAsync(u => u.Email.Equals(req.Email) && u.DataExclusao == null);
            ...
            if (!usuario.Ativo) {
                _appLogger.LoginSuccess(
                    usuario.Apelido,
                    false,
                    "Usuário não está ativo no sistema."
                );
                return ret.SetBadRequest()
                    .AddError(ETipoErro.ATENCAO, "Usuário não está ativo no sistema.");
            }
            ...
            if (!req.Password.VerifyPasswordHash(usuario.PasswordHash, usuario.PasswordSalt)) {
                _appLogger.LoginSuccess(
                    usuario.Apelido,
                    false,
                    "A senha digitada não está correta."
                );
                return ret.SetUnauthorized()
                    .AddError(ETipoErro.ERRO, "A senha digitada não está correta.");
            }

            var token = await _appDbContext.Tokens.FirstOrDefaultAsync(t =>
                t.UsuarioId.Equals(usuario.Id)
            );
            if (token == null) {
            ...
            var oauth2 = new OAuth2Dto {
                ...
                Usuario = new UsuarioOAuth2Dto {
                    UsuarioId = usuario.Id,
                    Apelido = usuario.Apelido,
                    SysAdm = usuario.SysAdm
                }
            };
        ...
        public async Task<bool> Logout() {
            var tokenModel = await _appDbContext.Tokens.FirstOrDefaultAsync(t =>
                t.Id.Equals(_requestContext.Authorization)
            );
            if (tokenModel != null) {
        ...
        private async Task<List<MenuSistemaDto>> GetMenus(Guid usuarioId) {
            ...
            if (menusUsuario == null) {
                menusUsuario = await _appDbContext
                    .MenusUsuarios.AsNoTracking()
                    .Include(m => m.MenuSistema)
                    .Where(m => m.UsuarioId == usuarioId && m.MenuSistema.Ativo)
                    .OrderBy(m => m.MenuSistema.Ordem)
                    .ToListAsync();
                await _cacheManager.Set(
                    cacheKey,
                    menusUsuario,
                    _cacheOption.MenuUsuario.Expiration
                );
            }

            var menuSistemaDto = new List<MenuSistemaDto>();
            foreach (
                var menu in menusUsuario
                    .Where(m => m.MenuSistema.MenuPredecessorId == null)
                    .OrderBy(m => m.MenuSistema.Ordem)
            ) {
            ...
            foreach (
                ...
                menuUsu ??= menuSistemaDto
                    .Where(m => m.children != null)
                    .SelectMany(m => m.children)
                    .ToList()
                    .Find(m => m.Id == menu.MenuSistema?.MenuPredecessorId);
