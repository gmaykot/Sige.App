...
namespace SIGE.Services.Services.Externo {
    ...
    public class IntegracaoCceeService(
        IHttpClient<CceeHttpClient> httpClient,
        AppDbContext appDbContext,
        IMapper mapper,
        IOptions<CceeOption> option
    ) : IIntegracaoCceeService {
        ...
        public async Task<Response> ListarMedicoes(IntegracaoCceeBaseDto req) {
            ...
            var empresa = await _appDbContext
                .Empresas.AsNoTracking()
                .Include(e => e.AgentesMedicao)
                .FirstOrDefaultAsync(e => e.Id == req.EmpresaId);
            if (empresa == null)
                return ret.SetBadRequest()
                    .AddError(ETipoErro.ATENCAO, "A empresa selecionada não existe.");
            ...
            foreach (var agente in empresa.AgentesMedicao) {
                var pontosMedicao = await _appDbContext
                    .PontosMedicao.AsNoTracking()
                    .Where(p => p.AgenteMedicao.Id == agente.Id)
                    .ToListAsync();
                foreach (var ponto in pontosMedicao) {
                    var xmlEnvelope = req.TipoMedicao.CreateSoapEnvelope(
                        agente.CodigoPerfilAgente,
                        credenciais.AuthUsername,
                        credenciais.AuthPassword,
                        ponto.Codigo,
                        req.Periodo.GetPrimeiraHoraMes(),
                        req.Periodo.GetUltimaHoraMes()
                    );
                    ...
                    try {
                        ...
                        if (res.IsSuccessStatusCode) {
                            ...
                            if (string.IsNullOrEmpty(content))
                                return ret.SetBadRequest()
                                    .AddError(
                                        ETipoErro.ATENCAO,
                                        "Retorno do serviço 'Listar Medidas' inválido."
                                    );

                            XDocument doc = XDocument.Parse(content.XmlSanitazer());
                            var medidas = doc.DescendantNodes()
                                .FirstOrDefault(n => n.ToString().Contains("outmedidas"));
                            ...
                            if (
                                resXml == null
                                || resXml.ListaMedidas != null
                                || !resXml.ListaMedidas.Any()
                            )
                                return ret.SetBadRequest()
                                    .AddError(
                                        ETipoErro.ATENCAO,
                                        "Nenhuma medida listada no serviço."
                                    );

                            medicoes.AddRange(
                                _mapper.Map<IEnumerable<IntegracaoCceeMedidasDto>>(
                                    resXml.ListaMedidas.OrderBy(n => n.PeriodoFinal).ToList()
                                )
                            );
                        }
                        else {
                            Log.Error(
                                "::: ERRO ListarMedicoes: RequestMessage => {0}, ReasonPhrase => {1}, xmlEnvelope => {2}",
                        ...
                        Log.Fatal(
                            "::: ERRO ListarMedicoes: Message => {0}, InnerException => {1}, xmlEnvelope => {2}, Exception => {3}",
                            ex.Message,
                            ex.InnerException?.Message,
                            xmlEnvelope,
                            ex.ToString()
                        );
                        return ret.SetInternalServerError()
                            .AddError(ETipoErro.ERRO, "Erro ao executar a integração com a Ccee")
        ...
        public async Task<Response> ListarMedicoesPorPonto(IntegracaoCceeBaseDto req) {
            ...
            var xmlEnvelope = req.TipoMedicao.CreateSoapEnvelope(
                req.CodAgente.Trim(),
                credenciais.AuthUsername.Trim(),
                credenciais.AuthPassword.Trim(),
                req.PontoMedicao.Trim(),
                req.Periodo.GetPrimeiraHoraMes(),
                req.Periodo.GetUltimaHoraMes()
            );
            var httpContent = new StringContent(xmlEnvelope, Encoding.UTF8, "text/xml");
            ...
            try {
                ...
                if (res.IsSuccessStatusCode) {
                    ...
                    XDocument doc = XDocument.Parse(content.XmlSanitazer());
                    var medidas = doc.DescendantNodes()
                        .FirstOrDefault(n => n.ToString().Contains("bmmedidas"));
                    var json = JsonConvert.SerializeXNode(medidas, Formatting.None, true);

                    var resXml = JsonConvert.DeserializeObject<IntegracaoCceeXmlDto>(json);
                    if (resXml == null || resXml.ListaMedidas == null || !resXml.ListaMedidas.Any())
                        return ret.SetBadRequest()
                            .AddError(ETipoErro.ATENCAO, "Nenhuma medida listada no período.");
                    ...
                    Log.Error(
                        "::: ERRO ListarMedicoesPorPonto: RequestMessage => {0}, ReasonPhrase => {1}, xmlEnvelope => {2}",
                        res.RequestMessage.ToString(),
                        res.ReasonPhrase,
                        xmlEnvelope
                    );
            ...
            if (resCcee.ListaMedidas.Any()) {
                ...
                return ret.SetNotFound()
                    .AddError(ETipoErro.INFORMATIVO, "SemNenhuma medida listada no período.");
            }
        ...
        public async Task<Response> ResultadoRelatorioBSv2(IntegracaoCceeBaseDto req) {
            ...
            try {
                ...
                if (res.IsSuccessStatusCode) {
                    ...
                    var valoresMaioresQueZero = retorno
                        .Body.ListarResultadoRelatorioResponse.Resultados.SelectMany(r =>
                            r.Valores.Select(v => v.Trim('\'').Split("','"))
                                .Where(campos => {
                        ...
                        .ToList();
                    return ret.SetOk()
                        .SetData(valoresMaioresQueZero)
                        .SetMessage("Integração efetuada com sucesso.");
                }
                else {
                    return ret.SetBadRequest()
                        .AddError(ETipoErro.ERRO, "Erro ao executar a integração com a Ccee")
