...
export class CalculoEconomiaService {
  ...
  public calcularAnalitico(
    relatorio: IRelatorioMedicao
  ): IValoresMedicaoAnalitico[] {
    if (!relatorio) return null;
    try {
      ...
      relatorio.valoresAnaliticos.forEach((val) => {
        ...
        var ret: IValoresMedicaoAnalitico = {
          ...
          venderCurtoPrazo: this.curtoPrazoUnitario(
            valores.venderCurtoPrazo,
            relatorio.valorVendaCurtoPrazo,
            relatorio.icms,
            true
          ),
        };
  ...
  private dentroTake() {
    ...
    return totalMedido >= takeMinimo && totalMedido <= takeMaximo
      ? true
      : false;
  }
  ...
  public calculaConsumoTotalUnitario(
    totalMedidoKWh: number,
    proinfa: number
  ): number {
