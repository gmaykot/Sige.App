...
export class RelatorioEconomiaComponent implements OnInit {
  @ViewChild("graficoContainer", { read: ViewContainerRef })
  graficoContainer!: ViewContainerRef;
  ...
  initChart() {
    ...
    // Preparar os dados
    const dadosBrutos =
      this.relatorioFinal.grafico?.linhas?.map((grupo) => ({
        nome: grupo.label,
        valor: grupo.valor,
        cor: cores.shift(),
      })) || [];
  ...
  async onSendEmail() {
    ...
    await this.getContatos(this.relatorioEconomia.cabecalho.fornecedorId);
    this.contatos.map((c) =>
      contatosSend.push({
  ...
  public async sendEmailAsPdf(contatosSend: IContatoEmail[]): Promise<void> {
    let graficoImagem = null;
    ...
    var file = this.relatorioEconomiaPdfService.blobPDF(
      this.relatorioFinal,
      graficoImagem
    );
    this.blobToBase64(file).then(async (result: string) => {
      if (result) {
        ...
        // Transforma a data em MM/yyyy
        const mesReferenciaDate = new Date(
          this.relatorioEconomia.mesReferencia
        );
        const descMesReferencia = `${(mesReferenciaDate.getMonth() + 1)
          .toString()
          .padStart(2, "0")}/${mesReferenciaDate.getFullYear()}`;

        var emailData: IEmailData = {
          contratoId: this.relatorioFinal.contratoId,
          relatorioEconomiaId: this.relatorioFinal.id,
          contato: contato,
          mesReferencia: this.relatorioEconomia.mesReferencia,
          descMesReferencia: descMesReferencia,
          descEmpresa: this.relatorioFinal.descGrupo,
          totalNota: Intl.NumberFormat("pt-BR", {
            maximumFractionDigits: 2,
            minimumFractionDigits: 2,
          }).format(this.relatorioFinal.totalMedido),
          relatorios: [result],
          contatosCCO: contatosSend.filter((c) => c.id != contato.id),
        };
        await this.emailService
          .sendEmailEconomia(emailData)
          .then(() =>
  ...
  validar() {
    this.dialogService
      .open(ValidacaoMedicaoComponent, {
        context: {
          observacao: this.relatorioEconomia.observacaoValidacao,
          validado: this.relatorioEconomia.validado,
          tipoRelatorio: "Economia",
        },
      })
  ...
  onHelp() {
    this.dialogService.open(AjudaOperacaoComponent, {
      context: { tipoAjuda: "relatorio-economia" },
    });
  }
}
