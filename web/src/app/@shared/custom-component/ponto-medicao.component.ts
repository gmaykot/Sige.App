            ...
            <div class="col-sm-12">
              <div class="form-group">
                <label for="inputFirstName" class="label"
                  >Agente de Medição*</label
                >
                <nb-select
                  fullWidth
                  placeholder="Selecione"
                  formControlName="agenteMedicaoId"
                >
                  <nb-option
                    *ngFor="let agente of agentes"
                    value="{{ agente.id }}"
                    >{{ agente.nome }}</nb-option
                  >
                </nb-select>
              </div>
            </div>
          ...
          <div class="row">
            <div class="col-sm-6">
              <div class="form-group">
                ...
                >
                  <nb-option *ngFor="let sg of segmento" value="{{ sg.id }}">{{
                    sg.desc
                  }}</nb-option>
                </nb-select>
              </div>
            ...
            <div class="col-sm-6">
              <div class="form-group">
                ...
                >
                  <nb-option *ngFor="let sg of conexao" value="{{ sg.id }}">{{
                    sg.desc
                  }}</nb-option>
                </nb-select>
              </div>
            ...
            <div class="col-sm-12">
              <div class="form-group">
                ...
                <nb-select
                  fullWidth
                  placeholder="Selecione"
                  formControlName="concessionariaId"
                >
                  <nb-option
                    *ngFor="let con of concessionarias"
                    value="{{ con.id }}"
                    >{{ con.descricao }}</nb-option
                  >
                </nb-select>
              </div>
            </div>
          ...
          <div class="row">
            <div class="col-sm-5">
              <div class="form-group">
                <nb-toggle
                  labelPosition="end"
                  formControlName="acumulacaoLiquida"
                  >Acumulação Líquida*</nb-toggle
                >
              </div>
            </div>
            <div class="col-sm-4">
              <div class="form-group">
                <nb-toggle labelPosition="end" formControlName="incideICMS"
                  >Incide ICMS*</nb-toggle
                >
              </div>
            </div>
            <div class="col-sm-3">
              <div class="form-group">
                <nb-toggle labelPosition="end" formControlName="ativo"
                  >Ativo*</nb-toggle
                >
              </div>
            </div>
...
export class PontoMedicaoComponent implements OnInit {
  ...
  constructor(
    protected dialogRef: NbDialogRef<PontoMedicaoComponent>,
  ...
  submit() {
    this.control.value.agenteMedicao = this.agentes.find(
      (a) => a.id == this.control.value.agenteMedicaoId
    )?.nome;
    this.control.value.descConcessionaria = this.concessionarias.find(
      (a) => a.id == this.control.value.concessionariaId
    )?.descricao;
    if (this.control.value.id == "") this.control.value.id = null;
